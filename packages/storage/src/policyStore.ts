...
export class PolicyStore {
  ...
  async get(tenantId: string, id: string): Promise<PolicyVersion | null> {
    const res = await this.pool.query(`SELECT * FROM policies WHERE tenant_id = $1 AND id = $2`, [
      tenantId,
      id,
    ]);
  ...
  async setStatus(
    ...
    const res = await this.pool.query(
      `UPDATE policies SET status = $3 WHERE tenant_id = $1 AND id = $2 RETURNING *`,
      [tenantId, id, status],
    );
    return res.rows[0] ? this.row(res.rows[0]) : null;
  }
  ...
  async activate(tenantId: string, id: string): Promise<PolicyVersion | null> {
    ...
    try {
      ...
      await client.query(
        `UPDATE policies SET status = 'archived' WHERE tenant_id = $1 AND name = $2 AND status = 'active'`,
        [tenantId, policy.name],
      );
      const res = await client.query(
        `UPDATE policies SET status = 'active', activated_at = now() WHERE tenant_id = $1 AND id = $2 RETURNING *`,
        [tenantId, id],
      );
  ...
  async rollback(
    tenantId: string,
    name: string,
  ): Promise<{ rolledBack: PolicyVersion | null; restored: PolicyVersion | null }> {
    const client = await this.pool.connect();
    try {
      await client.query("BEGIN");
      const cur = await client.query(
        `SELECT * FROM policies WHERE tenant_id = $1 AND name = $2 AND status = 'active' LIMIT 1`,
        [tenantId, name],
      );
      ...
      if (current) {
        await client.query(
          `UPDATE policies SET status = 'rolled_back' WHERE tenant_id = $1 AND id = $2`,
          [tenantId, current.id],
        );
        ...
        if (prior.rows[0]) {
          const r = await client.query(
            `UPDATE policies SET status = 'active', activated_at = now() WHERE id = $1 RETURNING *`,
            [prior.rows[0].id],
          );
  ...
  async getActiveArtifacts(tenantId: string): Promise<unknown[]> {
    const res = await this.pool.query(
      `SELECT artifact FROM policies WHERE tenant_id = $1 AND status = 'active'`,
      [tenantId],
    );
    return res.rows.map((r) =>
      typeof r.artifact === "string" ? JSON.parse(r.artifact) : r.artifact,
    );
  }

  async getShadowArtifacts(tenantId: string): Promise<unknown[]> {
    const res = await this.pool.query(
      `SELECT artifact FROM policies WHERE tenant_id = $1 AND status = 'shadow'`,
      [tenantId],
    );
    return res.rows.map((r) =>
      typeof r.artifact === "string" ? JSON.parse(r.artifact) : r.artifact,
