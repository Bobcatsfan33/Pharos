...
export class EscalationStore {
  ...
  /** Park an escalation. Idempotent on idempotencyKey: a repeat returns the existing one. */
  async create(input: {
    tenantId: string;
    recordSequence: number;
    context: Record<string, unknown>;
    ...
    const res = await this.pool.query<EscalationRow>(
      `INSERT INTO escalations (id, tenant_id, record_sequence, context, idempotency_key)
       VALUES ($1,$2,$3,$4,$5)
       ON CONFLICT (tenant_id, idempotency_key) DO UPDATE SET context = escalations.context
       RETURNING *`,
      [randomUUID(), input.tenantId, input.recordSequence, JSON.stringify(input.context), input.idempotencyKey],
    );
