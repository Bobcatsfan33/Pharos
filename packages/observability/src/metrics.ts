...
export class Histogram {
  ...
  private readonly counts = new Map<string, { labels: Labels; bucketCounts: number[]; sum: number; count: number }>();
  constructor(readonly name: string, readonly help: string, buckets: number[] = DEFAULT_BUCKETS) {
    this.buckets = [...buckets].sort((a, b) => a - b);
  }
  observe(value: number, labels: Labels = {}): void {
    const k = key(labels);
    const cur = this.counts.get(k) ?? { labels, bucketCounts: new Array(this.buckets.length).fill(0), sum: 0, count: 0 };
    cur.sum += value;
    cur.count += 1;
    for (let i = 0; i < this.buckets.length; i++) if (value <= this.buckets[i]!) cur.bucketCounts[i]! += 1;
    this.counts.set(k, cur);
  }
  render(): string {
    const lines = [`# HELP ${this.name} ${this.help}`, `# TYPE ${this.name} histogram`];
    for (const { labels, bucketCounts, sum, count } of this.counts.values()) {
      this.buckets.forEach((b, i) => lines.push(`${this.name}_bucket${renderLabels({ ...labels, le: String(b) })} ${bucketCounts[i]}`));
...
export class MetricsRegistry {
  ...
  readonly recordsSealed = new Counter("pharos_records_sealed_total", "Total sealed evidence records");
  readonly escalations = new Counter("pharos_escalations_total", "Total escalations parked");
  readonly verdictLatency = new Histogram("pharos_verdict_latency_ms", "End-to-end verdict latency (ms)");
  readonly errors = new Counter("pharos_errors_total", "Total errors by route");

  render(): string {
    return [this.verdicts, this.recordsSealed, this.escalations, this.verdictLatency, this.errors].map((m) => m.render()).join("\n\n") + "\n";
  }
}
