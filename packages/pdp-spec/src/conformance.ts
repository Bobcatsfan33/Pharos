...
function benign(over: Partial<PdpRequest> = {}): PdpRequest {
  return {
    ...
    liability: { mandate: null, oversightMode: "autonomous", blastRadius: { financialAmount: 0, currency: "USD", reversibility: "reversible" } },
    ...over,
  };
}
...
export async function runConformance(pdp: Pdp): Promise<ConformanceResult> {
  ...
  add("schema-valid", v1.valid, v1.valid ? "response conforms to the schema" : v1.errors.join("; "));
  ...
  add("decision-enum", ["allow", "block", "modify", "escalate"].includes(r1.decision), `decision=${r1.decision}`);
  ...
  add("deadline-echo", r3.latency.deadlineMs === 500, `latency.deadlineMs=${r3.latency.deadlineMs}`);

  // C4: an unmeetable deadline yields a fail-mode response consistent with reversibility.
  const rOpen = await pdp(benign({ deadlineMs: 0, liability: { mandate: null, oversightMode: "autonomous", blastRadius: { reversibility: "reversible" } } }));
  add("failmode-open", rOpen.failMode === "fail_open" && rOpen.latency.deadlineBreached, `reversible deadline=0 -> failMode=${rOpen.failMode}, breached=${rOpen.latency.deadlineBreached}`);
  const rClosed = await pdp(benign({ deadlineMs: 0, liability: { mandate: null, oversightMode: "human_in_loop", blastRadius: { financialAmount: 100, reversibility: "irreversible" } } }));
  add("failmode-closed", rClosed.failMode === "fail_closed" && rClosed.decision === "escalate", `irreversible deadline=0 -> failMode=${rClosed.failMode}, decision=${rClosed.decision}`);

  // C5: citations are well-formed.
  add("citations-shape", Array.isArray(r1.ruleCitations) && r1.ruleCitations.every((c) => typeof c.ruleId === "string" && typeof c.pack === "string"), "ruleCitations well-formed");
