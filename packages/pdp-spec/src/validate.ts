...
export function validatePdpResponse(obj: unknown): ValidationResult {
  ...
  if (!DECISIONS.includes(r.decision as string))
    errors.push(`decision must be one of ${DECISIONS.join("|")}`);
  if (!TIERS.includes(r.tierReached as number | string))
    errors.push("tierReached must be 1|2|3|'human'");
  if (typeof r.riskScore !== "number" || r.riskScore < 0 || r.riskScore > 1)
    errors.push("riskScore must be a number in [0,1]");
  if (!Array.isArray(r.ruleCitations)) errors.push("ruleCitations must be an array");
  else
    for (const c of r.ruleCitations)
      if (!c || typeof c.ruleId !== "string" || typeof c.pack !== "string")
        errors.push("each citation needs ruleId and pack");
  if (!(r.failMode === null || r.failMode === "fail_open" || r.failMode === "fail_closed"))
    errors.push("failMode must be null|fail_open|fail_closed");
  if (!(r.judgeVersion === null || typeof r.judgeVersion === "string"))
    errors.push("judgeVersion must be string|null");
  if (
    !r.latency ||
    typeof r.latency.totalMs !== "number" ||
    typeof r.latency.deadlineMs !== "number" ||
    typeof r.latency.deadlineBreached !== "boolean"
  ) {
  ...
  if (r.evidenceBinding) {
    ...
    if (typeof b.keyId !== "string" || typeof b.signature !== "string")
      errors.push("evidenceBinding needs keyId and signature");
  }
