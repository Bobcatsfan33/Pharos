...
export function compilePolicy(
  ...
  if (unparsed.length)
    warnings.push(`${unparsed.length} line(s) could not be interpreted and need manual rules.`);
...
function compileLine(packId: string, line: string, idx: number): PolicyRule | null {
  ...
  if (m) {
    return rule(
      ...
      packId,
      m[1]!,
      { judge: "finra-promissory", gte: 0.5 },
      `Tier-3 judge flags ${m[2]} language.`,
  ...
  if (m) {
    return rule(
      ...
      packId,
      m[1]!,
      { judge: "phi-in-context", gte: 0.5 },
      "Tier-3 judge flags PHI in message context.",
      "45 CFR 164.502(b)",
      0.9,
    );
  }
  ...
  m = line.match(
    /^(block|escalate|modify)\s+(.+?)\s+when\s+amount\s+(?:over|above|exceeds|greater than|>=?)\s*\$?([\d,]+)/i,
  );
  if (m) {
    ...
    return rule(
      ...
      m[1]!,
      cond,
      `${m[2]} exceeding ${amount} are ${m[1]!.toLowerCase()}ed.`,
      undefined,
      subj ? 0.9 : 0.7,
    );
  }
  ...
  if (m) {
    const subj = subjectToCondition(m[1]!) ?? {
      field: "action.type",
      op: "contains",
      value: m[1]!.trim(),
    };
  ...
  m = line.match(
    /^(block|escalate)\s+(.+?)\s+when\s+(.+?)\s+(?:contains|includes|mentions)\s+"(.+)"/i,
  );
  if (m) {
    return rule(
      id,
      packId,
      m[1]!,
      { field: `action.payload.${m[3]!.trim()}`, op: "contains", value: m[4]! },
      `${m[2]} mentioning "${m[4]}".`,
      undefined,
      0.6,
    );
