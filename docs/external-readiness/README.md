...
2. Give the owner the matching packet, repository evidence snapshot, and relevant deployment facts.
   When the engagement requires a runnable image, first verify that the live GitHub release-control
   audit passes. An authorized release maintainer may then create a semver prerelease tag
   (`vMAJOR.MINOR.PATCH-rc.N`) from the assessed commit. The protected image workflow returns the
   signed digest and receipt; prerelease tags do not publish the npm or PyPI SDKs.
