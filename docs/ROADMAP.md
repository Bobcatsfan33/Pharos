...
| 2 | **Lantern** | Real decision engine + measured latency | p99 < 800ms @ 1k verdicts/s for 1h (existing result is the linear development baseline; transformer customer-topology re-benchmark remains open); Tier-3 registry supports linear development models and production-required hash-verified ONNX transformers; chaos-proven fail modes; 10 verdicts replay bit-identical |
| 3 | **Causeway** | SDKs, gateway, escalation round trip | Reference + unmodified agent both block→escalate→human verdict→atomic resume claim; gateway delivery survives restart and uses target idempotency; middlewares pass conformance; $25k mandate blocks $30k; 3 design partners |
