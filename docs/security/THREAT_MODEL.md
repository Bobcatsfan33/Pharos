...
| **T** | Silent corruption of the stored body | **Accepted risk [#77](https://github.com/Bobcatsfan33/Pharos/issues/77)** — no verify-on-read re-hash; `reconcile()` for orphans unimplemented; `ensureBucket` doesn't assert Object Lock on a pre-existing bucket. Authoritative reads come from Postgres; WORM is the tamper-proof backstop |
...
| **I** | Exfiltrate private key material | **AWS KMS**: private key never leaves the HSM ([`awsKms.ts:20`](../../packages/core/src/signing/awsKms.ts)). **LocalKms**: **Accepted risk [#78](https://github.com/Bobcatsfan33/Pharos/issues/78)** — plaintext PKCS8 in `0o600` JSON, dev/test only; nothing yet refuses `local-kms` in prod |
...
| **S** | Guess/abuse the admin token | `requireAdminToken` ([`auth.ts:109`](../../services/api/src/auth.ts)); 503 if unset. **Accepted risk [#75](https://github.com/Bobcatsfan33/Pharos/issues/75)** — non-constant-time compare, no rotation/expiry |
...
| **S/I** | Clickjacking / no CSP / open dashboard | **Accepted risk [#79](https://github.com/Bobcatsfan33/Pharos/issues/79)** — no CSP/HSTS/X-Frame-Options headers, no browser auth gate, hardwired to `demo-tenant` |
...
| [#78](https://github.com/Bobcatsfan33/Pharos/issues/78) | LocalKms plaintext keys; local-kms not refused in prod | KMS | Accepted for dev; boot-guard filed |
