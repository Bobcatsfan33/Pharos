...
export function registerEscalationRoutes(app: FastifyInstance, platform: Platform): void {
  app.get<{ Params: { tenantId: string } }>(
    "/v1/tenants/:tenantId/escalations",
    async (request, reply) => {
      ...
      const humanRecord = await platform.store.append({
        ...
        verdict: {
          ...
          latency: {
            totalMs: 0,
            perTier: {},
            deadlineMs: platform.config.api.verdictDeadlineMs,
  ...
  app.post<{ Params: { tenantId: string; id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (escalation.status === "pending") {
        return reply.send({
          success: true,
          data: { claimed: false, status: "pending", escalation },
          error: null,
        });
      }
      if (escalation.status === "rejected") {
        return reply.send({
          success: true,
          data: { claimed: false, status: "rejected", escalation },
          error: null,
        });
      }
