...
export function registerPolicyRoutes(app: FastifyInstance, platform: Platform): void {
  ...
  async function historicalContexts(tenantId: string, window: number): Promise<EvalContext[]> {
    ...
    return range.map(({ record }) => {
      const request: VerdictRequest = {
        ...
        liability: record.content.liability,
      };
  ...
  app.post<{ Params: { tenantId: string } }>(
    "/v1/tenants/:tenantId/policies/compile",
    async (request, reply) => {
      ...
      const policy = await platform.policyStore.createDraft({
        tenantId,
      ...
      return reply.status(201).send({
        success: true,
        data: { policy, warnings: compiled.warnings, unparsed: compiled.unparsed },
        error: null,
      });
    },
  );
  ...
  app.get<{ Params: { tenantId: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply.send({
        success: true,
        data: {
          policies: await platform.policyStore.list(tenantId),
          shippedPacks: Object.values(SHIPPED_PACKS).map((p) => ({
            packId: p.packId,
            version: p.version,
  ...
  app.post<{ Params: { tenantId: string; id: string } }>(
    "/v1/tenants/:tenantId/policies/:id/shadow",
    async (request, reply) => {
      const { tenantId, id } = request.params;
      const principal = await requireAuth(platform, request, reply, "policies:write", tenantId);
      if (!principal) return reply;
  ...
  app.post<{ Params: { tenantId: string; id: string } }>(
    "/v1/tenants/:tenantId/policies/:id/activate",
    async (request, reply) => {
      const { tenantId, id } = request.params;
      const principal = await requireAuth(platform, request, reply, "policies:write", tenantId);
      if (!principal) return reply;
      ...
      if (policy.status !== "shadow") {
        return reply.status(409).send({
          ...
          data: null,
          error: {
            code: "must_shadow_first",
            message: "Promote to shadow and review divergence before activating.",
          },
        });
      }
  ...
  app.post<{ Params: { tenantId: string; name: string } }>(
    "/v1/tenants/:tenantId/policies/:name/rollback",
    async (request, reply) => {
...
function mostSevereAcross(artifacts: PA[], ctx: EvalContext): VerdictDecision {
  ...
  for (const artifact of artifacts)
    for (const m of evaluateArtifact(artifact, ctx)) d = worse(d, m.decision);
  return d;
}
