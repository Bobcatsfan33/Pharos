...
const CreateTenantSchema = z.object({
  tenantId: z.string().min(1).regex(/^[a-z0-9][a-z0-9-]*$/, "tenantId must be kebab-case"),
...
export function registerAdminRoutes(app: FastifyInstance, platform: Platform): void {
  ...
  app.post("/v1/admin/tenants", async (request, reply) => {
    ...
    const { record, plaintext } = await platform.apiKeys.create(tenant.tenantId, "bootstrap tenant-admin", ALL_PERMS);
  ...
  app.post<{ Params: { tenantId: string }; Body: { status: string } }>(
    ...
    async (request, reply) => {
      ...
      if (status !== "active" && status !== "suspended") {
        return reply.status(400).send({ success: false, data: null, error: { code: "invalid_status" } });
      }
      await platform.tenants.setStatus(request.params.tenantId, status);
      return reply.send({ success: true, data: { tenantId: request.params.tenantId, status }, error: null });
    },
  );

  app.delete<{ Params: { tenantId: string } }>("/v1/admin/tenants/:tenantId", async (request, reply) => {
    if (!requireAdminToken(platform, request, reply)) return reply;
  ...
  app.post<{ Params: { tenantId: string } }>("/v1/tenants/:tenantId/keys", async (request, reply) => {
    ...
    if (scopes.length !== parsed.data.scopes.length) {
      return reply.status(400).send({ success: false, data: null, error: { code: "unknown_scope" } });
    }
    const { record, plaintext } = await platform.apiKeys.create(tenantId, parsed.data.name, scopes);
    return reply.status(201).send({ success: true, data: { keyId: record.keyId, plaintext, scopes }, error: null });
  });

  app.get<{ Params: { tenantId: string } }>("/v1/tenants/:tenantId/keys", async (request, reply) => {
    const { tenantId } = request.params;
    const principal = await requireAuth(platform, request, reply, "keys:manage", tenantId);
    if (!principal) return reply;
  ...
  app.post<{ Params: { tenantId: string; keyId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!rotated) return reply.status(404).send({ success: false, data: null, error: { code: "not_found" } });
      return reply.status(201).send({
        success: true,
        data: { keyId: rotated.record.keyId, plaintext: rotated.plaintext, scopes: rotated.record.scopes, rotatedFrom: keyId },
        error: null,
      });
    },
  );
  ...
  app.get<{ Params: { tenantId: string } }>("/v1/tenants/:tenantId/audit", async (request, reply) => {
    const { tenantId } = request.params;
  ...
  app.get<{ Params: { tenantId: string } }>("/v1/tenants/:tenantId/audit/verify", async (request, reply) => {
    const { tenantId } = request.params;
    const principal = await requireAuth(platform, request, reply, "audit:read", tenantId);
    if (!principal) return reply;
