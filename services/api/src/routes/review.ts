...
function toReviewRecord(e: Escalation): ReviewRecord {
  const ctx = e.context as {
    verdict?: { riskScore?: number; ruleCitations?: Array<{ ruleId: string; pack: string }> };
  };
...
export function registerReviewRoutes(app: FastifyInstance, platform: Platform): void {
  ...
  app.post<{ Params: { tenantId: string; id: string }; Body: { reviewer?: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!assigned)
        return reply
          .status(409)
          .send({ success: false, data: null, error: { code: "not_pending" } });
      return reply.send({ success: true, data: { escalation: assigned }, error: null });
    },
  );

  app.get<{ Params: { tenantId: string } }>(
    "/v1/tenants/:tenantId/review/analytics",
    async (request, reply) => {
      const { tenantId } = request.params;
      const principal = await requireAuth(platform, request, reply, "reviews:read", tenantId);
      if (!principal) return reply;
      ...
      return reply.send({
        success: true,
        data: { ...summarize(records), queueDepth: depths },
        error: null,
      });
    },
  );
  ...
  app.get<{ Params: { tenantId: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply.send({
        success: true,
        data: { ruleCandidates: draftRuleCandidates(items) },
        error: null,
      });
    },
  );
  ...
  app.get<{ Params: { tenantId: string } }>(
    "/v1/tenants/:tenantId/review/notifications",
    async (request, reply) => {
      const { tenantId } = request.params;
      const principal = await requireAuth(platform, request, reply, "reviews:read", tenantId);
      if (!principal) return reply;
