...
export function registerSealRoutes(app: FastifyInstance, platform: Platform): void {
  ...
  app.post<{ Params: { tenantId: string } }>(
    ...
    async (request, reply) => {
      ...
      const hold = await platform.evidenceOps.createHold({
        tenantId,
        ...parsed.data,
  ...
  app.get<{ Params: { tenantId: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply.send({
        success: true,
        data: { holds: await platform.evidenceOps.listHolds(tenantId) },
        error: null,
      });
    },
  );
  ...
  app.post<{ Params: { tenantId: string; id: string } }>(
    "/v1/tenants/:tenantId/holds/:id/release",
    async (request, reply) => {
  ...
  app.get<{ Params: { tenantId: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply.send({
        success: true,
        data: { anchors: await platform.evidenceOps.listAnchors(tenantId) },
        error: null,
      });
    },
  );
  ...
  // --- Claims packs ---
  app.post<{ Params: { tenantId: string } }>(
    "/v1/tenants/:tenantId/claims-packs",
    async (request, reply) => {
      const { tenantId } = request.params;
      const principal = await requireAuth(platform, request, reply, "records:export", tenantId);
      if (!principal) return reply;
  ...
  app.post<{ Params: { tenantId: string; id: string } }>(
    "/v1/tenants/:tenantId/claims-packs/:id/seal",
    async (request, reply) => {
      const { tenantId, id } = request.params;
      const principal = await requireAuth(platform, request, reply, "records:export", tenantId);
      if (!principal) return reply;
        ...
        for (let seq = pack.fromSequence; seq <= pack.toSequence; seq++) {
          if (await platform.evidenceOps.isUnderHold(tenantId, seq)) {
            return reply.status(409).send({
              success: false,
              data: null,
              error: { code: "redaction_disabled_under_hold", sequence: seq },
            });
          }
        }
      }
      ...
      if (records.length === 0)
        return reply
          .status(400)
          .send({ success: false, data: null, error: { code: "empty_range" } });
  ...
  app.post<{ Params: { tenantId: string; id: string }; Body: { releasedTo?: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!released)
        return reply
          .status(409)
          .send({ success: false, data: null, error: { code: "not_sealed" } });
      await platform.accessAudit.record({
        tenantId,
        actor: principal.subject,
        actorKind: principal.kind,
        action: "share",
        resource: `claims-pack:${id}`,
        metadata: { releasedTo },
      });
  ...
  app.get<{ Params: { tenantId: string; id: string } }>(
    "/v1/tenants/:tenantId/claims-packs/:id",
    async (request, reply) => {
      const { tenantId, id } = request.params;
      const principal = await requireAuth(platform, request, reply, "records:read", tenantId);
      if (!principal) return reply;
  ...
  app.get<{ Params: { tenantId: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply.send({
        success: true,
        data: { packs: await platform.evidenceOps.listPacks(tenantId) },
        error: null,
      });
    },
  );
  ...
  // --- Regulatory exports ---
  app.get<{
    Params: { tenantId: string; format: string };
    Querystring: { from?: string; to?: string };
  }>("/v1/tenants/:tenantId/exports/:format", async (request, reply) => {
    const { tenantId, format } = request.params;
    const principal = await requireAuth(platform, request, reply, "records:export", tenantId);
    if (!principal) return reply;
    const valid: RegulatoryFormat[] = ["finra", "eu_ai_act_12", "sr_11_7"];
    if (!valid.includes(format as RegulatoryFormat))
      return reply
        .status(400)
        .send({ success: false, data: null, error: { code: "unknown_format" } });
