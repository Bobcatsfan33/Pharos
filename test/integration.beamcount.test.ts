...
beforeAll(async () => {
  try {
    ...
    auth["x-api-key"] = (await platform.apiKeys.create(TENANT, "bc", ["actions:write", "records:read", "reviews:act", "reviews:read", "audit:read", "tenants:manage"])).plaintext;
  } catch (err) {
...
describe("Beam Count — assurance, readiness gate, underwriter feed", () => {
  it("computes verified accuracy from >=1000 audits with a confidence interval", async (ctx) => {
    ...
    const candidates = Array.from({ length: 1050 }, (_, i) => ({ recordSequence: i, pack: ["finra", "hipaa", "core"][i % 3]!, decision: "allow" }));
    ...
    const res = await app!.inject({ method: "GET", url: `/v1/tenants/${TENANT}/assurance`, headers: auth });
  ...
  it("the risk profile computes from records and carries the measured assurance bound", async (ctx) => {
    ...
    for (let i = 0; i < 4; i++) {
      await app!.inject({ method: "POST", url: "/v1/actions", headers: auth, payload: { tenantId: TENANT, action: { type: "payment.transfer", agentId: "t", payload: { amount: 20000 + i } }, liability: { mandate: null, oversightMode: "human_in_loop", blastRadius: { financialAmount: 20000 + i, currency: "USD", reversibility: "irreversible" }, modelMetadata: null } } });
    }
    const res = await app!.inject({ method: "GET", url: `/v1/tenants/${TENANT}/risk-profile`, headers: auth });
  ...
  it("the readiness gate blocks the underwriter feed on mandate-coverage failure, exception unblocks", async (ctx) => {
    ...
    const readiness = await app!.inject({ method: "GET", url: `/v1/tenants/${TENANT}/readiness`, headers: auth });
    expect(readiness.json().data.readiness.blocked).toBe(true);
    expect(readiness.json().data.readiness.checks.find((c: { id: string }) => c.id === "mandate-coverage").passed).toBe(false);

    // The underwriter feed (an external release) is blocked.
    const blockedFeed = await app!.inject({ method: "GET", url: `/v1/tenants/${TENANT}/underwriter-feed`, headers: auth });
    ...
    const ex = await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/readiness/exceptions`, headers: auth, payload: { checkId: "mandate-coverage", reason: "remediation in progress" } });
    expect(ex.statusCode).toBe(200);

    // Now the gate passes and the versioned feed is released.
    const ready = await app!.inject({ method: "GET", url: `/v1/tenants/${TENANT}/readiness`, headers: auth });
    expect(ready.json().data.readiness.blocked).toBe(false);
    const feedRes = await app!.inject({ method: "GET", url: `/v1/tenants/${TENANT}/underwriter-feed`, headers: auth });
