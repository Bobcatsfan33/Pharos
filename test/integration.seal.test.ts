...
beforeAll(async () => {
  try {
    ...
    key = (await platform.apiKeys.create(TENANT, "seal", ["actions:write", "records:read", "records:export", "audit:read"])).plaintext;
    auth["x-api-key"] = key;
  } catch (err) {
...
async function submit(payload: Record<string, unknown>) {
  return app!.inject({
    ...
    payload: {
      ...
      liability: { mandate: { id: "m", scope: "pay", limits: { maxAmount: 1_000_000 }, grantor: "cfo", expiresAt: null, version: "1" }, oversightMode: "human_on_loop", blastRadius: { financialAmount: Number(payload.amount ?? 0), currency: "USD", reversibility: "reversible" }, modelMetadata: null },
    },
  });
}
...
describe("Seal — incident drill, offline verification, redaction under hold", () => {
  it("runs the full drill end to end", async (ctx) => {
    ...
    const hold = await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/holds`, headers: auth, payload: { name: "INC-42", reason: "regulatory inquiry", fromSequence: 0, toSequence: 3 } });
    expect(hold.statusCode).toBe(201);

    // 3. Redaction is disabled under hold.
    const draftRedacted = await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/claims-packs`, headers: auth, payload: { incident: "INC-42", audience: "claims_adjuster", fromSequence: 0, toSequence: 3, redactFields: ["to"] } });
    const sealRedacted = await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/claims-packs/${draftRedacted.json().data.pack.id}/seal`, headers: auth });
    ...
    const draft = await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/claims-packs`, headers: auth, payload: { incident: "INC-42", audience: "outside_counsel", fromSequence: 0, toSequence: 3, redactFields: [] } });
    const packId = draft.json().data.pack.id;
    const sealed = await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/claims-packs/${packId}/seal`, headers: auth });
    ...
    const released = await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/claims-packs/${packId}/release`, headers: auth, payload: { releasedTo: "counsel@firm.example" } });
    ...
    const fetched = await app!.inject({ method: "GET", url: `/v1/tenants/${TENANT}/claims-packs/${packId}`, headers: auth });
    ...
    await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/holds/${holdId}/release`, headers: auth });
    const draft2 = await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/claims-packs`, headers: auth, payload: { incident: "INC-42", audience: "claims_adjuster", fromSequence: 0, toSequence: 3, redactFields: ["to", "memo"] } });
    const sealed2 = await app!.inject({ method: "POST", url: `/v1/tenants/${TENANT}/claims-packs/${draft2.json().data.pack.id}/seal`, headers: auth });
    ...
    for (const format of ["finra", "eu_ai_act_12", "sr_11_7"]) {
      const exp = await app!.inject({ method: "GET", url: `/v1/tenants/${TENANT}/exports/${format}`, headers: auth });
      expect(exp.statusCode).toBe(200);
      expect(exp.json().data.export.format).toBeTruthy();
    }

    // 10. The pack release/read is in the access audit chain.
    const auditVerify = await app!.inject({ method: "GET", url: `/v1/tenants/${TENANT}/audit/verify`, headers: auth });
