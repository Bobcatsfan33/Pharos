...
7. **Publishing hardening (new, 2026-07-23, from S2-T1b).** The first manual npm publish of `@getpharos/sdk` 0.1.0 shipped a broken `exports` map (0.1.1 fixes it). Three lessons, now binding:
   - **(a) `publishConfig.exports` is pnpm-only — never rely on it.** The npm CLI silently ignores `publishConfig.exports`, so a package published with `npm publish` (as the OIDC trusted-publishing flow does) keeps whatever `exports` is in the manifest. Point `exports` at `./dist` in the manifest itself; do not use `publishConfig` to rewrite it at pack time.
   - **(b) Every release AC must include installing the packed tarball and importing it**, not merely building it. `pnpm build` + `tsc` were green while the published package was unusable, because the tarball's `exports` resolved to `.ts` source. The release checklist now packs the tarball, `npm i` it into a scratch project, and imports the entry point. (Consequence for the repo: publishable `exports` → `./dist`, each package has a `files` field, and both `ci.yml` and `release.yml` run `pnpm build:publishable` before `pnpm -r typecheck`, since with no tsconfig `paths` the cross-package typecheck resolves through `exports`.)
   - **(c) GitHub App tokens cannot modify `.github/workflows/` files.** A change that must touch a workflow cannot be made by an App-authored PR (its token lacks the `workflows` permission); route workflow changes through a maintainer-scoped token/PAT. This is why the S2-T1b workflow reorder landed as a separate maintainer-driven commit on the fix PR.

8. **KMS emulator + cross-provider keyId continuity (new, 2026-07-24, from S3-T0/S3-T3).** Two findings from the "Real KMS" sprint:
   - **(a) The CI KMS emulator is `nsmithuk/local-kms`, not LocalStack.** The S3-T0 spike found LocalStack's `latest` image now **requires a paid license token** (exits 55 without one), which our no-secrets CI cannot supply; pinning an EOL LocalStack is fragile. `nsmithuk/local-kms` is license-free, ~36 MB, and passes the full ECC_NIST_P256 flow incl. offline `node:crypto` verify. The S3-T1 AC's "LocalStack KMS in CI" is therefore satisfied by nsmithuk/local-kms.
   - **(b) keyIds must stay globally unique across a provider switch.** The "no data migration" property (S3-T3) relies on `<name>#v<n>` being globally unique and the keyset being additive. Both providers independently default a fresh keyName to `#v1`, so a naive local-kms→aws-kms switch would mint two different keys both called `<name>#v1` and break the merged keyset. The migration must **continue the version sequence** (local `#v1` → aws `#v2`); S3-T3 added `AwsKms.provisionVersion(keyName, version)` (a provider-specific migration helper, NOT a `SigningProvider` interface change) and the runbook (`docs/runbooks/key-rotation.md`) documents the procedure. No change to the seal/canonical/chain formats.

9. **Stacked-PR merge protocol (new, 2026-07-24, from the Sprint 3 landing).** #64/#65/#66 were stacked; #64 was squash-merged to main, but GitHub only retargets a stacked PR's base to main when the base *branch is deleted* on merge. The branches weren't deleted, so #65 and #66 — CI-green and fully reviewed — merged into their stack parents, not main, and the error was only caught by a post-sprint audit of main. The merge-button error was the tech lead's. Fixes, now binding: (a) repo setting **"Automatically delete head branches" is ON** so stacks retarget correctly; (b) before merging ANY PR, verify its `base` ref is `main` at merge time; (c) after landing a sprint, verify main actually contains the sprint's changes (`git log main` or the diff view) before reporting it landed. S3-T2/T3 were landed on main by cherry-picking the reviewed commits (content unchanged, re-verified by CI).

10. **Eval-validity requirements (new, 2026-07-26, from Sprint 5; BINDING on all Phase-2+ eval and model work).** The eval harness is the measuring stick every future judge is graded against, so a flattering-but-invalid eval is worse than none. These ten requirements are binding for Sprint 6 onward; Sprint 5 implemented all of them, and `@pharos/judge-eval` + `docs/benchmarks/judge-evals.md` are the reference:
   - **(a) Hard negatives dominate.** Each concern's negative set must be majority near-miss (a compliant example that shares surface features with a violation — e.g. a past-performance statement without an implied guarantee, a de-identified PHI-adjacent message, an in-mandate large transfer), with a pre-committed near-miss taxonomy. Easy negatives inflate precision and hide the failure modes the judge actually meets in production. Sprint 5 shipped 90/89/97% hard-negative composition.
   - **(b) State the base rate; lead with PR-AUC.** A balanced eval reports precision that is *not* operational precision. Every report carries a base-rate box and, absent a defensible deployment prevalence, publishes prevalence scenarios rather than implying the balanced number is operational. PR-AUC (prevalence-sensitive) is the lead ranking metric; ROC-AUC is secondary.
   - **(c) Confidence intervals on every metric; the gate is statistical.** Reuse the Wilson-score machinery from `@pharos/assurance` for proportions and a seeded bootstrap for derived metrics. The regression gate (S5-T4, `pnpm judges:gate`) fails only when the entire paired-delta interval is worse than a pre-committed tolerance — never on point estimates. "The new model beat the baseline" is a statistical claim or it is not made.
   - **(d) Leakage detection is near-duplicate, not exact-match.** For any text model (and especially bag-of-words), n-gram overlap between eval and training data is leakage even with zero exact matches. The gate measures n-gram containment / trigram Jaccard, is CI-enforced, and must itself be proven to FIRE on a seeded exact and near-duplicate (a "0 hits" assertion with no proof the detector works is meaningless). This gate caught its own author's first FINRA templates mid-Sprint-5.
   - **(e) Operating thresholds are frozen and content-hashed.** Metrics are reported at a locked per-concern operating point (hash-pinned), plus the full curve. A candidate model is compared at the frozen point — it may not pick its own optimal threshold, which would be cheating the comparison.
   - **(f) The scorer interface is async and batchable.** `scoreBatch` (order-preserving, configurable batch size) so a served/remote judge (Sprint 6 ONNX/transformer, possibly with latency) drops into the same harness without a rewrite.
   - **(g) Negative controls.** Every report includes trivial floors (always-positive, seeded-random) so a model's numbers — especially adversarial recall — have a floor to beat.
   - **(h) The committed dataset is the source of truth; the generator is provenance, not a reproduction recipe.** The dataset file + its content hash is authoritative; the generation script documents how it was made. Regeneration is not expected to be bit-reproducible (and if an external LLM is ever used, is not reproducible at all). Do not treat "rerun the generator" as a way to reconstruct the eval set.
   - **(i) Generator ≠ downstream model family; multilingual suites are native.** Document the generator's identity and flag any shared family with a downstream distilled model as a known limitation. Sprint 5 used engineer-authored templates (no external LLM), which is the strongest answer — it shares no generator family with any future model. Spanish/German suites are natively authored, never machine-translated (translated regulatory language is stilted and artificially easy); native-speaker sign-off is a tracked human gate.
   - **(j) Labels have defensible provenance.** Labels are template properties grounded in cited authority (FINRA 2210(d)(1); HIPAA 45 CFR §160.103 / §164.514(b); SR 11-7 / payment-ops), not model-inferred, and a deterministic stratified 10% sample is emitted for qualified-compliance-human review (a tracked gate, distinct from engineer spot-check).
