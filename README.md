...
Every consequential agent action flows through a single ingestion surface — one SDK call, or a no-code MCP/gateway proxy. The verdict cascade evaluates it across tiers (deterministic rules, statistical risk scoring, then a served judge model) within an 800ms envelope, with deterministic short-circuiting and engineered fail-open / fail-closed paths. Local development defaults to the measured linear baseline; production configuration now requires and preloads every hash-verified ONNX transformer judge before opening a listener. The transformer artifacts are still awaiting the independent efficacy, calibration, drift, model-card, and production-latency evidence listed in [docs/LIMITATIONS.md](docs/LIMITATIONS.md).
...
- **Signing** — a pluggable `SigningProvider`; local Ed25519 for development and production-required AWS KMS P-256 with rotation, outage circuit breaking, and chain continuity.
...
> **Known gaps and stand-ins.** Several components still require production promotion evidence (most importantly the ONNX judges, plus deployment-specific KMS/TSA trust). Local development intentionally retains linear/KMS/TSA stand-ins. Every limitation is listed in **[docs/LIMITATIONS.md](docs/LIMITATIONS.md)**. Read it before evaluating Pharos against a production bar.
...
**Sprint 2 (Lantern) — complete.** A real tiered decision cascade (Tier 1 deterministic rules → Tier 2 statistical risk → Tier 3 served judge models with a versioned registry), a hard 800ms deadline manager with engineered fail-open/fail-closed semantics, and a reproducibility (replay) harness. The registry now dispatches either the measured linear development baseline or CPU ONNX transformers through the same contract; production startup requires and verifies all three transformer artifacts and never silently falls back. The **[Tier-3 judge eval baseline](docs/benchmarks/judge-evals.md)** and **[encoding system evaluation](docs/benchmarks/system-encoding-eval.md)** remain honest about the evidence boundary. The p99 3.7ms / ~5,400 verdicts/sec headline was measured with the linear baseline and is not a transformer production claim; the production-topology re-benchmark remains open. See [docs/decision-cascade.md](docs/decision-cascade.md).
